...
python -m LArWrapper -n 20 --nskip=100 --debug=True  --flist="filelist.txt"\
 --delivery_method=list --workflow_method=interactive -c pduneana_Prod4_data_reco2_6GeV.fcl\
   --user $USER --appFamily=protoduneana --appName=test --appVersion=$PROTODUNEANA_VERSION\
    --dataTier=root-tuple --dataStream=physics --namespace=dune --TFileName=$OUTFILE 
