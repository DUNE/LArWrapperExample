...
if len(imd.keys()) == 1:
  print " this looks like sam_metadata_dumper output"
  
  filename = imd.keys()[0]
  md =imd[filename]
  md["file_name"] = filename
  required = ["file_name","file_type","data_tier","application.name","application.version", "application.family","event_count","data_stream","runs"]
...
if md.has_key("application.version"):
  nover =  md["application.version"] == "null"
