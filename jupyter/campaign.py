...
thecampaigns=["FDVDProd1","PDSPProd4","PDSPProd4a","ALL"]
thedetectors=["fardet-vd","protodune-sp","protodune-dp","coldboxALL"]

month = sys.argv[1]
first = sys.argv[1]
last = sys.argv[2]

out = open("Campaign_report_"+first+"_"+last+"_%d.tex"%MConly,'w')
...
if not MConly:
  ...
  for expt in thedetectors:
    out.write("\\\\\n")
    for stream in ["physics","ALL"]:
      if short and stream == "ALL":
        continue
      out.write(divide)
      for tier in ["raw","full-reconstructed","pandora_info","reco-recalibrated","ALL"]:
...
for expt in thedetectors:
  ...
  for campaign in thecampaigns:
    if short and campaign == "ALL":
      continue
    for tier in ["simulated","detector-simulated","full-reconstructed","hit-reconstructed","pandora_info","ALL",]:
      if short and tier == "ALL":
        continue
